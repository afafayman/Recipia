...
async function getCachedRecipes(ingredients) {
  try {
    ...
    if (bestMatch) {
      ...
      console.log(`✅ Cache hit! ${Math.round(bestScore * 100)}% match`);
      return bestMatch.recipes;
    }
    return null;
  } catch (e) {
    console.warn('Cache lookup failed silently:', e.message);
    return null;
  }
}
...
async function cacheRecipes(ingredients, recipesData) {
  try {
    ...
    console.warn('Cache save failed silently:', e.message);
  }
}
...
async function trackSearch(recipes) {
  try {
    ...
    console.warn('trackSearch failed silently:', e.message);
  }
}
...
async function fetchGlobalStats() {
  try {
    ...
    console.warn('fetchGlobalStats failed silently:', e.message);
